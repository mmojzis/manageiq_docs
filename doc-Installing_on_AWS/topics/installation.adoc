...
+
----
$ aws iam put-role-policy --roRle-name vmimport --policy-name vmimport --policy-document file://role-policy.json
----
+
...
[NOTE]
====
The S3 bucket must be in the same region where you want to import the snapshot.
====
+
----
...
+
----
 $ aws ec2 describe-import-snapshot-tasks --import-task-ids import-ami-AMI_ID_GOT_FROM_RESPONSE
----
+
